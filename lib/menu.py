...
try:
    ...
    class LibraryInfoFrame(tk.Frame):
        def __init__(self, parent, lib_dir, libs):
            ...
            tk.Label(self, text="[ SYSTEM INVENTORY PAGE ]", font=("Consolas", 14), 
                     fg=COLORS["accent"], bg=COLORS["bg_dark"]).pack(expand=True)

try:
    from .settings import SettingsFrame
except ImportError:
    # نسخة احتياطية في حال عدم وجود ملف settings.py
    class SettingsFrame(tk.Frame):
        def __init__(self, parent, colors):
            super().__init__(parent, bg=colors["bg_dark"])
            tk.Label(self, text="[ SETTINGS PAGE NOT FOUND ]", font=("Consolas", 14), 
                     fg="red", bg=colors["bg_dark"]).pack(expand=True)
...
class LibraryMenu:
    def __init__(self, filtered_libs, input_type="None", standalone_mode=False):
        ...
        # --- إعداد المسارات البرمجية الذكية ---
    ...
    def setup_ui(self):
        ...
        tk.Label(self.sidebar, text="Maltego Transforms", font=("Segoe UI Black", 12), 
                 fg=self.COLORS["accent"], bg=self.COLORS["sidebar"], pady=10).pack()

        btn_style = {"bg": self.COLORS["sidebar"], "fg": self.COLORS["text_main"], "font": ("Segoe UI", 10, "bold"), 
                     "bd": 0, "cursor": "hand2", "anchor": "w", "padx": 25, "pady": 15,
        ...
        # إضافة زر SETTINGS فوق الـ ABOUT
    ...
    def create_main_page(self):
        ...
        self.counter_label = tk.Label(frame, text="", font=("Consolas", 10), 
                                      fg=self.COLORS["accent"], bg=self.COLORS["bg_dark"])
        self.counter_label.pack(pady=5)

        tk.Button(frame, text="EXECUTE SELECTED TRANSFORMS", command=self.on_confirm, 
                  bg=self.COLORS["accent"], fg="black", font=("Segoe UI", 11, "bold"), 
                  bd=0, height=2, cursor="hand2").pack(fill="x", padx=30, pady=20)
    ...
    def update_list(self):
        ...
        for lib in self.filtered_libs:
            ...
            if search in name.lower():
                ...
                tk.Label(card, text=name.upper(), font=("Segoe UI", 10, "bold"), 
                         fg=self.COLORS["text_main"], bg=self.COLORS["bg_card"], anchor="w").pack(side="left", padx=10)
                chk = tk.Checkbutton(card, variable=self.vars[folder], bg=self.COLORS["bg_card"], 
                                     activebackground=self.COLORS["bg_card"], selectcolor="black",
                                     fg=self.COLORS["accent"], command=self.update_counter)
                chk.pack(side="right")
        self.update_counter()
    ...
    def create_about_page(self):
        ...
        tk.Label(frame, text="BAYLAK MANAGER v3.0\nProfessional OSINT Toolkit", 
                 ...
                 bg=self.COLORS["bg_dark"]).place(relx=0.5, rely=0.5, anchor="center")
