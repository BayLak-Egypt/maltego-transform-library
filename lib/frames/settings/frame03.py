...
class SettingsFrame(tk.Frame):
    ...
    def setup_ui(self):
        ...
        self.update_cb = tk.Checkbutton(config_group, text='ENABLE AUTOMATIC UPDATES', variable=self.auto_update, bg=self.COLORS['bg_card'], fg='white', selectcolor=self.COLORS['bg_dark'], font=('Consolas', 10), activebackground=self.COLORS['bg_card'])
        self.update_cb.pack(anchor='w')
        save_btn = tk.Button(self, text='SAVE & CHECK UPDATES', command=self.handle_save, bg=self.COLORS['accent'], fg=self.COLORS['bg_dark'], bd=0, font=('Consolas', 9, 'bold'), pady=12, cursor='hand2')
